...
const PublicationAdd = () => {
  const name = 'News';
  const description = 'Add to publication Items';
  ...
  const log = () => {
    if (editorRef.current) {
      ...
      setDescriptions(editorRef.current.getContent());
    }
  };
  ...
  const postPublication = ()=>{
    ...
    if(!thumbnail){
       notify('warn', "Please add document thumbnail");
      return;
    }

    if(!size){
       notify('warn', "Please add document file size");
      return;
    }

    if(!url){
       notify('warn', "Please provide the document url");
      return;
    }

    if(!fileType){
       notify('warn', "Please provide the document file type");
      return;
    }
    ...
    fetch(`${baseURL}publications`, {method: 'POST', body, headers})
          ...
          <Card.Body>
            <Form className="mb-n3">
              <div className="mb-3 filled">
                ...
                <Form.Control type="text" onChange={(e)=>setTitle(e.target.value)} placeholder="Publication Title" />
              </div>
              ...
              <div className="mb-3 filled">
                <CsLineIcons icon="tag" />
                <Form.Control type="text" onChange={(e)=>setSize(e.target.value)} placeholder="Document Size" />
              </div>
...
*/}
        ...
        <div className="text-center">
          <div className="shadow d-inline-block">
            ...
            <Button variant="primary" onClick={()=>{postPublication()}} className="btn-icon btn-icon-end" size="lg">
              <span>Publish</span>
