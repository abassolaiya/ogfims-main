...
const NewsEditor = () => {
  ...
  const [featuredImage, setFeaturedImage] = useState(location.state.data?.featured_image);
 ...
 /* 
   if(!featuredImage){
    ...
    const body = JSON.stringify({ "title": title, "content": editorRef.current.getContent(), featured_image: featuredImage});
     // console.log(body);
    fetch(`${baseURL}posts/${postid}`, {method: 'PUT', body, headers})
...
*/}
           ...
           <div>
            ...
            <div className="mb-3 mx-auto position-relative" id="imageUpload">
              <img src={baseURL+featuredImage} alt="user" className="rounded-xl border border-separator-light border-4 sw-11 sh-11" id="contactThumbModal" />
              <Button
                size="sm"
